...
flags.DEFINE_string("model_path", "models/chunk_mlp/best.pt", "Path to trained model weights")
...
def main(_):
    ...
    seeds = np.random.randint(0, 2**31, size=FLAGS.num_seeds).tolist() if FLAGS.random_seeds else list(range(FLAGS.num_seeds))
    for i, seed in enumerate(seeds):
        ...
        while not (terminated or truncated):
            ...
            # --- 4. PREDICT ---
            with torch.no_grad():
                pred_norm_chunk = model(image_tensor, state_tensor_norm)
            ...
            raw_action = ensemble_current_action(step, chunk_predictions, horizon=horizon, decay=ensemble_decay)
        ...
        if success:
            success_count += 1
            print(f"Episode {i + 1}/{FLAGS.num_seeds} (seed={seed}) - SUCCESS (Took {step} steps)")
        else:
            print(f"Episode {i + 1}/{FLAGS.num_seeds} (seed={seed}) - FAILED (Reached {step} steps)")
