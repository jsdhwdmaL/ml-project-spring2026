#!/usr/bin/env python3
"""Fine-tune the 5-D Push-T zarr ACT model on collected DAgger episodes.

Mirrors ``scripts/act_dagger_finetune.py`` but:
  * targets the 5-D state checkpoint trained by ``scripts/act_train_pusht.py``
  * mixes collected DAgger episodes (saved by
    ``scripts/act_dagger_collect_og_data.py``) with the original Push-T zarr
    buffer (``data/pusht/pusht_cchi_v7_replay.zarr``) instead of LeRobot.
"""
...
from data.dataloader_pusht import (
    PushTZarrDataset,
    load_pusht_zarr,
    split_episode_indices as split_zarr_episode_indices,
)
...
class FinetuneConfig:
    model_path: str = "models/act_og_data_50_epochs/latest.pt"
    data_dir: str = "data/act_dagger_og_data"
    zarr_path: str = "data/pusht/pusht_cchi_v7_replay.zarr"
    output_dir: str = "models/act_dagger_og_data"
...
class ACTNPZStepDataset(Dataset):
    """Step-wise Dataset over DAgger NPZ episodes (5-D state)."""

    def __init__(
        self,
        images: np.ndarray,        # (N, H, W, 3) uint8
        states: np.ndarray,        # (N, state_dim) float32
        step_indices: np.ndarray,  # (M,) int64
...
def _load_successful_data(config: FinetuneConfig, expected_state_dim: int):
...
def _resolve_arch_from_checkpoint(config: FinetuneConfig, ckpt_config: dict):
    ...
    if config.ensemble_decay is None:
        config.ensemble_decay = float(ckpt_config.get("ensemble_decay", 0.05))


def _load_original_pusht_zarr_data(config: FinetuneConfig):
    """Load the local Push-T zarr buffer and wrap it as train/val datasets.

    Returns the same dict layout as the LeRobot loader in the original
    finetune script so the rest of the pipeline is unchanged.
    """
    arrays = load_pusht_zarr(config.zarr_path)
    images = arrays["images"]                # (N, 96, 96, 3) uint8
    states = arrays["states"]                # (N, 5) float32
    actions = arrays["actions"]              # (N, 2) float32
    episode_index = arrays["episode_index"]  # (N,) int64

    action_chunks, action_is_pad = build_action_chunks_by_episode(
        actions, episode_index, config.horizon
    )

    train_idx, val_idx = split_zarr_episode_indices(episode_index, config.val_ratio, config.seed)

    train_dataset = PushTZarrDataset(images, states, action_chunks, action_is_pad, train_idx)
    val_dataset = PushTZarrDataset(images, states, action_chunks, action_is_pad, val_idx)
...
def train(config: FinetuneConfig) -> None:
    ...
    if config.wandb:
        ...
        wandb_run = wandb.init(
            ...
            tags=["act", "dagger_finetune", "og_data"],
        )
    ...
    ckpt_state_mean = np.asarray(base_checkpoint["state_mean"], dtype=np.float32)
    state_dim = int(ckpt_state_mean.shape[0])
    if state_dim != 5:
        raise ValueError(
            f"This script targets the 5-D Push-T zarr ACT model, but checkpoint has "
            f"state_dim={state_dim}. Use scripts/act_dagger_finetune.py for 2-D models."
        )

    images, states, actions, episode_index, data_stats = _load_successful_data(config, expected_state_dim=state_dim)
    print(
    ...
    original_data = None
    if config.include_original_data:
        original_data = _load_original_pusht_zarr_data(config)
        print(
            "Loaded original Push-T zarr data | "
    ...
    try:
        for epoch in range(1, config.epochs + 1):
            ...
            checkpoint = {
                ...
                "source_zarr_path": config.zarr_path,
